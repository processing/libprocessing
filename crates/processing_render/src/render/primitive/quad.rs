...
fn quad_path(x1: f32, y1: f32, x2: f32, y2: f32, x3: f32, y3: f32, x4: f32, y4: f32) -> Path {
    let mut b = Path::builder();
    b.begin(Point::new(x1, y1));
    b.line_to(Point::new(x2, y2));
    b.line_to(Point::new(x3, y3));
    b.line_to(Point::new(x4, y4));
    b.end(true);
    b.build()
}
...
pub fn quad(
    ...
    if matches!(mode, TessellationMode::Fill) {
        simple_quad(mesh, x1, y1, x2, y2, x3, y3, x4, y4, color);
    } else {
        let path = quad_path(x1, y1, x2, y2, x3, y3, x4, y4);
